...
def opt_trfmr(tfconfig):
    ...
    class CustomModel(tf.keras.Model):
        ...
        def train_step(self, data_combined, y=None, first_batch=None):
            ...
            with tf.GradientTape() as tape:
                ...
                tf.print("####")
                tf.print(f"{predictions.shape}:predictions.shape")
                tf.print(f"{tfconfig.batch_size}:tfconfig.batch_size")
                loss = compute_loss(tfconfig.label[0], predictions, tfconfig.batch_size)
            gradients = tape.gradient(loss, self.trainable_variables)
            self.train_auc.update_state(tfconfig.label[0], predictions)
        ...
        def test_step(self, data_combined, y=None, first_batch=None):
            ...
            predictions = self.transformer(tfconfig)  # prediction, pweight, rweight, p_cr_weight, r_cr_weight
            loss = compute_loss(tfconfig.label[0], predictions, tfconfig.batch_size)
            self.test_auc.update_state(tfconfig.label[0], predictions)
    ...
    if tfconfig.training == 1:
        ...
        combined_dataset_test = combined_dataset_test.repeat(1).batch(test_batch)
    ...
    if tfconfig.training == 1:
        callbacks = [cp_callback_with_date, cp_callback_no_date, tensorboard_callback]
    ...
    with strategy.scope():
        ...
        def compute_loss(labels, prediction, global_batch_size):
            bce = loss_object(labels, prediction)
            per_example_loss = tf.cast(bce, dtype="float32")
            return tf.nn.compute_average_loss(per_example_loss)
            # return tf.nn.compute_average_loss(per_example_loss, global_batch_size * 1.25)

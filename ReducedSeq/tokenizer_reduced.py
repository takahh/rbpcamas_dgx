...
# -------------------------------------------------------------------
# $$$$$$$$$$$$$$$ SEE HEREEEEEEEE!!!!!!!!!
moder = "unknown_shared_reduced_broad_share"  # shared or all
# moder = "shared"  # shared or all
...
if moder in ["all", "known_shared", "shared"]:
    path = "/Users/mac/Desktop/t3_mnt/reduced_RBP_camas/data/"
    input = f"{path}/known_protein_reduced/{moder}_{reduce_level}/"
...
elif moder == "unknown_shared_reduced":
    path = "/Users/mac/Desktop/t3_mnt/reduced_RBP_camas/data/"
    input = f"/Users/mac/Desktop/t3_mnt/reduced_RBP_camas/data/unknown_reduced/"
elif moder == "attn_ana":
...
elif moder == "unknown_shared_reduced_broad_share":
    path = "/Users/mac/Desktop/t3_mnt/reduced_RBP_camas/data/"
    input = f"/Users/mac/Desktop/t3_mnt/reduced_RBP_camas/data/unknown_reduced_broad_share/"

# pot path and output path
if moder == "unknown_shared_reduced":
    hbpot_path = f"{path}attn_arrays_hb/"
    pipot_path = f"{path}attn_arrays_pi/"
    output = f"{path}training_data_tokenized/"
elif moder == "unknown_shared_reduced_broad_share":
    hbpot_path = f"{path}attn_arrays_hb_broad_share/"
    pipot_path = f"{path}attn_arrays_pi_broad_share/"
    output = f"{path}training_data_tokenized_broad_share_red_{reduce_level}/"
elif moder != "attn_ana":
    hbpot_path = f"{path}attn_arrays_hb/{moder}_{reduce_level}/"
    pipot_path = f"{path}attn_arrays_pi/{moder}_{reduce_level}/"
    output = f"{path}training_data_tokenized/"
else:
...
def main(gnum):  # gnum (gpu_id, protein_group_id)
    ...
    for i in thisrange:
        if moder in ["unknown_shared_reduced", "unknown_shared_reduced_broad_share"]:  # when unknown
            arr = np.load(f"{input}/{group}/{i}.npy", allow_pickle=True)
            # /Users/mac/Desktop/t3_mnt/reduced_RBP_camas/data/attn_arrays_hb_broad_share/0/0.npz
            hbpot_arr = np.load(f"{hbpot_path}/{group}/{i}.npz", allow_pickle=True)["pot"]
            pipot_arr = np.load(f"{pipot_path}/{group}/{i}.npz", allow_pickle=True)["pot"]
        else:
            try:
        ...
        # # ogroup: "all5_small_tape" when known protein
        if WRITE == 1:
            np.savez_compressed(f"{output}/{group}/{i}", proid=pro_id_arr, protok=pro_token_arr, rnatok=rna_token_arr, p_tape_arr=p_tape_arr,
...
if __name__ == '__main__':
    print(moder)
    if moder in ["unknown_shared_reduced", "unknown_shared_reduced_broad_share"]:

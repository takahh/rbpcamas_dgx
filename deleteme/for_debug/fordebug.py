...
class Tfconfig():
    ...
    def update(self, task_id):
        ...
        self.training_npz_dir = BASE_PATH + "data/training_data_tokenized/all5_small/"
...
def scaled_dot_product_attention(k, q, v, tffig, one_on_rna, w_aug=None, w_at=None):
    ...
    if one_on_rna == 2 and tffig.use_attn_augument == 1:
        ...
        try:
            ...
            tf.print(f"table_to_augment.shape {table_to_augment.shape}")
            tf.print(f"table_to_augment_hb.shape {table_to_augment_hb.shape}")
            tf.print(f"table_to_augment_pi.shape {table_to_augment_pi.shape}")
            tf.print(f"tffig {vars(tffig)}")
...
def opt_trfmr(tfconfig):
    ...
    current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    # train_log_dir = BASE_PATH + "/logs/" + tfconfig.keyword + "/" + tfconfig.taskname + "_" + current_time + "/"
    ...
    model.fit(combined_dataset_train, epochs=tfconfig.max_epoch, callbacks=[CheckPoint(), ctensorboard_callback])
    # model.fit(combined_dataset_train, epochs=tfconfig.max_epoch, callbacks=[CheckPoint(), ctensorboard_callback],

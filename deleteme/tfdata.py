...
class Tfconfig():
    ...
    def update(self, task_id):
        ...
        self.taskname = f"timetest{task_id}"
...
class Embedders(tf.keras.layers.Layer):  # RNA
    ...
    def call(self, tf_cfg, one_when_rna):  # tfcfg, RCODES, i, 1, self.rna_out
        ...
        # padding
        if one_when_rna == 0:
            zero_pad = np.array([[0] * tf_cfg.d_model] * (maxlen - x.shape[1]))
...
def get_three_veclists(tffg):
    ...
    tf.print(f"cross mask took {time.time() - start}")
    start = time.time()
    ...
    tf.print(f"label took {time.time() - start}")
    start = time.time()
...
def opt_trfmr(tfconfig):
    ...
    for epoch in range(max_epoch):
        ...
        for i in range(tfconfig.max_file_num):
            ...
            tf.print(f"get stat tables took {time.time() - start}")

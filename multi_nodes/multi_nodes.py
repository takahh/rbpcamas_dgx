...
########################################################################
BASE_PATH = "/Users/mac/Documents/transformer_tape_dnabert/"
...
class Tfconfig():
    ...
    def update(self, task_id):
        ...
        self.checkpoint_path = f"{BASE_PATH}chpoint/{self.keyword}/chpt_"
        self.checkpoint_dir = f"{BASE_PATH}chpoint/{self.keyword}/"
...
def opt_trfmr(tfconfig):
    ...
    class CustomModel(tf.keras.Model):
        ...
        def train_step(self, data_combined, y=None, first_batch=None):
            ...
            for i in range(0, 1):
                ...
                auc.update_state(tfcfig.label, predictions)
        ...
        def test_step(self, data_combined, y=None):
            ...
            for i in range(0, 1):
                ...
                auc.update_state(tfcfig.label, predictions)
                if hvd.rank() == 0:
                    tf.print("ROC_DATA:" + ":" + str(tf.nn.softmax(predictions).numpy()).replace('\n', '') + "-" + str(tfcfig.label).replace('\n', ''))

            return {"test_loss": loss_avg.result(), "test_auc": auc.result()}

        @tf.function()
        # @tf.function(input_signature=train_step_signature)
        def predict(self, data_combined, y=None):
            tfcfig = tfconfig
            tfcfig.training_boolean = False
            for i in range(0, 1):
                datalist = [x for x in data_combined][0]
                datalist = [x[i] for x in datalist]
                tfcfig.proid = datalist[0]
                tfcfig.protok = datalist[1]
                tfcfig.rnatok = datalist[2]
                tfcfig.statpot_hb = datalist[3]
    ...
    combined_dataset_train = combined_dataset.take(int(100))
    # combined_dataset_train = combined_dataset.take(int(16))
    # combined_dataset_dummy = combined_dataset.take(int(tfconfig.datafiles_num * 0.016))
    # combined_dataset_validation = combined_dataset.skip(int(tfconfig.datafiles_num * 0.8)).skip(int(tfconfig.datafiles_num * 0.1))
    # combined_dataset_test = combined_dataset.skip(int(16)).take(int(2))
    combined_dataset_test = combined_dataset.skip(int(100)).take(int(10))

    # batchfy, shard, prefetch
    combined_dataset_train = combined_dataset_train.repeat(1).batch(1).with_options(options)
    combined_dataset_test = combined_dataset_test.repeat(1).batch(1).with_options(options)
    ...
    if tfconfig.use_attn_augument == 1:
        aug_weight_at = tf.compat.v1.get_variable(
            "aug_weight_at",
            shape=[1],
            trainable=True,
            dtype=tf.float32,
            initializer=initializer)
        aug_weight_aug = tf.compat.v1.get_variable(
            "aug_weight_aug",
            shape=[1],
            trainable=True,
            dtype=tf.float32,
        ...
        if tfconfig.clip_coeff == 1:
            aug_weight_aug = tf.sigmoid(aug_weight_aug)
            aug_weight_at = tf.sigmoid(aug_weight_at)
        tfconfig.aug_weight_aug = aug_weight_aug
        tfconfig.aug_weight_at = aug_weight_at
    # run model
    auc = tf.keras.metrics.AUC()
    loss_avg = tf.keras.metrics.Mean(name='train_loss')
    model = CustomModel()
    ...
    #             model.save_weights(f"{checkpoint_path}ckpt")
    #             tf.print(f"{hvd.rank()} : saving...")
    #             checkpoint.save(checkpoint_path)
    callbacks = []
    # callbacks = [CustomCheckPoint(), ctensorboard_callback]

    model.compile(optimizer=optimizer)
    # model.compile(optimizer=optimizer, experimental_run_tf_function=False)
    # model.fit(combined_dataset_train, epochs=tfconfig.max_epoch, callbacks=callbacks, steps_per_epoch=4000//hvd.size(), verbose=verbose)
    # model.fit(combined_dataset_train, epochs=tfconfig.max_epoch)
    model.fit(combined_dataset_train, epochs=tfconfig.max_epoch, validation_data=combined_dataset_validation)
    # model.fit(combined_dataset_train, epochs=tfconfig.max_epoch, callbacks=cp_callback, validation_data=combined_dataset_validation)
    output = model.evaluate(combined_dataset_test)

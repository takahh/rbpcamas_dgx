...
def opt_trfmr(tfconfig):
    ...
    # combined_dataset_validation = combined_dataset_validation.repeat(1).batch(1).with_options(options).prefetch(tf.data.AUTOTUNE)
    combined_dataset_train = combined_dataset_train.repeat(1).batch(tfconfig.num_of_gpu).with_options(options).prefetch(tf.data.AUTOTUNE)
    combined_dataset_test = combined_dataset_test.repeat(1).batch(1).with_options(options).prefetch(tf.data.AUTOTUNE)
    ...
    model.fit(combined_dataset_train, epochs=tfconfig.max_epoch, callbacks=callbacks, verbose=verbose)
    ...
    output = model.predict(combined_dataset_test)

...
class FinalLayer(tf.keras.layers.Layer):
    ...
    def call(self, x1, x2, cffg):
        x = tf.keras.layers.Average()([tf.reduce_mean(x1[0], axis=1), tf.reduce_mean(x2, axis=1)])
        x = self.finallayer(x)
        return x
...
class Transformer(tf.keras.Model):
    ...
    def call(self, tf_cfg):
        ...
        prediction = self.final_layer.call(rout, pout, tf_cfg)
        return prediction
...
def objective(trial):
    ...
    # # with constraint
    # config.num_heads = 2
    # config.self_player_num = 4
    # config.self_rlayer_num = 4
    # config.cross_layer_num = 1

    # without constraint
    config.num_heads = 2 ** trial.suggest_int("heads", 1, 2)
    config.self_player_num = 2 ** trial.suggest_int("play", 0, 3)
    config.self_rlayer_num = 2 ** trial.suggest_int("rlay", 0, 3)
    config.cross_layer_num = 2 ** trial.suggest_int("crlay", 0, 3)

    config.datafiles_num = 1000
    config.rna_dff = 2 ** trial.suggest_int("rdff", 6, 7)
    config.pro_dff = 2 ** trial.suggest_int("pdff", 6, 7)
    config.cross_dff = 2 ** trial.suggest_int("crdff", 6, 7)
...
if __name__ == '__main__':
    ...
    study = optuna.create_study(pruner=optuna.pruners.MedianPruner(), storage='sqlite:///../optuna_study0422.db',
                            load_if_exists=True, study_name="honban0422")

...
class Tfconfig():
    ...
    def update(self, task_id):
        ...
        self.checkpoint_path = BASE_PATH + "_" + self.taskname
...
def scaled_dot_product_attention(k, q, v, tffig, one_on_rna, w_aug=None, w_at=None):
    ...
    if tffig.two_d_softm == 1:
        ...
        if attention_weights.shape[2] == tffig.max_pro_len:
...
def opt_trfmr(tfconfig):
    ...
    train_log_dir = BASE_PATH + "/logs/" + tfconfig.taskname + "_" + current_time + "/"
    ...
    class CustomModel(keras.Model):
        ...
        def train_step(self, data_combined, y=None):
            ...
            for i in range(0, 50, bsize):
                ...
                tf.print(f"datalist[1].shape {datalist[1].shape}")
            ...
            return {"train_loss": loss_avg.result(), "train_auc": auc.result()}
    ...
    tf.print(f"os.path.isdir(checkpoint_path) {os.path.isdir(checkpoint_path)}")
    ...
    current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    train_log_dir = BASE_PATH + "/logs/{tfconfig.keyword}/{tfconfig.taskname_" + current_time + "/"
    ...
    combined_dataset_train = combined_dataset_train.repeat(1000).batch(tfconfig.num_of_gpu, drop_remainder=True).with_options(options)
    ...
    with strategy.scope():
        ...
        model.fit(combined_dataset_train, epochs=tfconfig.max_epoch, callbacks=[CheckPoint(), ctbks])
        model.evaluate(combined_dataset_test, epochs=tfconfig.max_epoch, callbacks=[ctbks])
...
if __name__ == '__main__':
    ...
    config.num_of_gpu = int(sys.argv[20])
    config.max_pro_len = int(sys.argv[21])
    config.task_identifier = sys.argv[1: 19]
    config.task_identifier = "_".join(config.task_identifier)
    #config.task_identifier = config.task_identifier.replace(", ", "_").replace("[", "").replace("]", "").replace("'", "")
    config.update(config.task_identifier + "_" + config.keyword)
    opt_trfmr(config)

...
def main(mode):
    attn_dict = get_attn_dict(mode)
    RNA_seq_dict = get_seq_dict(RNA_seq_file)
    protein_dict = get_seq_dict(protein_seq_file)
    zero_row = [0] * 4000

    if mode == "HB":
        attn_out_dir = f"{path}attn_arrays/{benchmark_set}/"
    else:
        attn_out_dir = f"{path}attn_arrays_pi/{benchmark_set}/"
        ...
        for lines in f.readlines():
            ...
            for pcount in range(3680):
                ...
                if pcount < len(protein_seq):
                    ...
                    row_list = zero_row
                attn_to_write_list.append(row_list)
            attn_to_write_list = np.array(attn_to_write_list)

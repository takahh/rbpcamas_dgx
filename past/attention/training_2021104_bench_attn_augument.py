...
class Tfconfig():
    ...
    def update(self):
        ...
        self.taskname = f"_{self.benchname}_cv{self.group_to_ignore}_{self.max_batch_num}files_auth_order_no_parenthesis_lr{self.init_lr}"
...
def get_stat_pot_arr(tfcfg):
    arr = np.load(f"{tfcfg.statistical_pot_path}{tfconfig.pair_name}.npz", allow_pickle=True)
    return arr["data"]


def scaled_dot_product_attention(k, q, v, mask, location, tfconfig, smask=None, block_num=None):  # q, k, v, mask, mha_num, smask
    ...
    # add astatistical potentials to the first Decoder layer
    if smask is not None:
        if block_num == 0:
            table_to_augment = get_stat_pot_arr(tfconfig)
            attention_weights += table_to_augment
            attention_weights = tf.cast(tf.nn.softmax(attention_weights, axis=-1), tf.float32)
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inp, tar, training, tfconfig, enc_padding_mask, cross_padding_mask_small, cross_padding_mask):
        inp = tf.cast(inp, tf.float32)
        enc_output = self.encoder(inp, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)
        dec_output, attention_weights = self.decoder(tar, enc_output, training, tfconfig, cross_padding_mask, cross_padding_mask_small)
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, q, k, mask, training, tfconfig=None, block_num=None, mha_num=None, smask=None):
        ...
        scaled_attention, attention_weights = scaled_dot_product_attention(
            q, k, v, mask, mha_num, tfconfig, smask, block_num)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training,  # x:RNA, enc_output:protein
             cross_padding_mask, tfconfig, block_num=None, cross_mask_small=None):
        out1 = x
        #                                       v,          q,      k
        attn2, attn_weights_block2 = self.mha2(out1, enc_output, out1, cross_padding_mask, training, tfconfig, block_num, 2, cross_mask_small)
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, tfconfig, cross_padding_mask, cross_padding_mask_small):  # x = tar previously
        attention_weights = {}
        x = self.first(x)
        x = tf.cast(x, tf.float32)
        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float32))
        ...
        for i in range(self.num_layers):
            ...
            x, block1 = self.dec_layers[i](x, enc_output, training, cross_padding_mask, tfconfig, i, cross_padding_mask_small)
...
def opt_trfmr(tfconfig):
    ...
    @tf.function()
    def train_step(inp, m, record_num, tfconfig):  # tar : (100, 30, 170)
        ...
        with tf.GradientTape() as tape:
            predictions, _ = transformer(inp, tar_inp,  # "call" is called here
                                                     True,  # inp, tar, training, tfconfig, enc_padding_mask,
                                                     tfconfig,
                                                     enc_padding_mask,
                                                     cross_mask_small,
                                                     cross_mask)
        ...
        with open(tfconfig.pairfile) as f:
            for lines in f.readlines():
                ...
                if record_num not in training_list:
                    ...
                    loss, auc = train_step(ele, auc, record_num, tfconfig)
                    if loss is None:
                        continue
                    ...
                    # tf.print(f"RECORD {record_num}, AUC {auc.result().numpy()}")
                record_num += 1

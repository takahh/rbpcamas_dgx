...
class Tfconfig():
    ...
    def update(self):
        ...
        self.taskname = f"_{self.benchname}_cv{self.group_to_ignore}_{self.max_batch_num}files_opposite_order_no_parenthesis_lr{self.init_lr}"
...
def opt_trfmr(tfconfig):
    ...
    def loss_function(labellist, predictions):
        if labellist[0] == 1:
            ...
            new_labellist = [0, 1]
        bce = tf.nn.softmax_cross_entropy_with_logits(new_labellist, predictions[0], axis=-1)

...
def get_three_veclists(inp, record_num, tfconfig3):  # inp = [prot_name, rna_name, label]
    ...
    # 4. make a label list
    label_list = [int(inp[2][0].strip())]
    if label_list == [1]:
        tfconfig.new_label_list = tf.convert_to_tensor([[0, 1]], dtype="float32")
    else:
        tfconfig.new_label_list = tf.convert_to_tensor([[1, 0]], dtype="float32")
...
if __name__ == '__main__':
    ...
    config.num_heads = int(sys.argv[11])
    config.use_attn_augument = int(sys.argv[4])

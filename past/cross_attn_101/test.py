...
class Tfconfig():
    ...
    def update(self, task_id):
        ...
        self.checkpoint_path = f"{BASE_PATH}_{self.taskname}_timetest"
...
def scaled_dot_product_attention(k, q, v, tffig, one_on_rna, w_aug=None, w_at=None):
    ...
    tf.print(f"matmul qk took {time.time() - start}")
    start = time.time()
    ...
    if one_on_rna == 2 and tffig.use_attn_augument == 1:
        ...
        augtable_toadd = tf.math.multiply(w_aug, tf.cast(table_to_augment, dtype="float32"))
        scaled_attention_logits = tf.math.add(scaled_attention_logits, augtable_toadd)

    # add large negative values at padds according to the mask (except RNA)
    if one_on_rna != 1:
        if scaled_attention_logits.shape[-1] != mask.shape[-1]:
            mask = tf.transpose(mask, perm=[0, 1, 3, 2])
        scaled_attention_logits = tf.math.add(scaled_attention_logits, mask * -1e9)

    # 2D softmax
    if tffig.two_d_softm == 1:
        attention_weights = tf.cast(tf.keras.activations.softmax(scaled_attention_logits, axis=[2, 3]), tf.float32)
        attention_weights = tf.math.multiply(attention_weights, attention_weights.shape[2])
    else:
    ...
    tf.print(f"matmul v took {time.time() - start}")
...
def opt_trfmr(tfconfig):
    ...
    train_log_dir = f"{BASE_PATH}/logs/{current_time}_timetest/"

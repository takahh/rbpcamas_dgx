...
def opt_trfmr(tfconfig):
    ...
    class CustomModel(keras.Model):
        ...
        def train_step(self, data_combined):
            ...
            # for i in range(0, 50, tfcfig.batch_size):
            for i in range(0, 50, bsize):
                ...
                tf.print(f"loss {loss_avg.result().numpy()}, auc {auc.result().numpy()}")
    ...
    # batchfy
    combined_dataset = combined_dataset.shuffle(buffer_size=10).batch(tf.convert_to_tensor(1, tf.int64), drop_remainder=True)
    # data_to_check = list(combined_dataset.as_numpy_iterator())
    # tf.print(f"combined_dataset shape {data_to_check.shape}")
    ...
    # model.fit(combined_dataset, epochs=tfconfig.max_epoch, callbacks=[CheckPoint()])

...
class Tfconfig():
    ...
    def update(self, task_id):
        ...
        if self.training == 0:
            ...
            tf.print(f"training is {self.training}, boolean is {self.training_boolean}")
        else:
...
def scaled_dot_product_attention(k, q, v, tffig, one_on_rna, w_aug=None, w_at=None):
    ...
    if one_on_rna == 2 and tffig.use_attn_augument == 1:
        ...
        table_to_augment = tf.repeat(table_to_augment, repeats=[multiply_num, multiply_num], axis=0)
...
def opt_trfmr(tfconfig):
    ...
    for epoch in range(max_epoch):
        ...
        if tfconfig.training == 1:
            filenum_range = range(tfconfig.max_file_num)
        else:
            filenum_range = range(tfconfig.max_file_num, tfconfig.max_file_num + tfconfig.testfiles_num)
...
if __name__ == '__main__':
    ...
    config.keyword = sys.argv[16]
    config.training = int(sys.argv[17])
    config.testfiles_num = int(sys.argv[18])
    task_identifier = f"{sys.argv[1: 17]}"

...
def opt_trfmr(tfconfig):
    ...
    class CustomModel(keras.Model):
        ...
        def train_step(self, inpdata):
            data_combined = inpdata[0]
            tfcfig = tfconfig
            tfcfig.proid = data_combined[0]
            tfcfig.protok = data_combined[1]
            # tfconfig.protok = tfconfig.protok[tf.newaxis, :]
            # tf.print(f"tfconfig.protok.shape after {tfconfig.protok}")
            tfcfig.rnatok = data_combined[2]
            # tfconfig.rnatok = tfconfig.rnatok[tf.newaxis, :]
            tfcfig.statpot_hb = data_combined[3]
            tfcfig.statpot_pi = data_combined[4]
            tfcfig.self_pro_mask_list = data_combined[5]
            tfcfig.cross_padding_mask_list = data_combined[6]
            tfcfig.label = data_combined[7]
            ...
            if tfcfig.accum_gradient is None:  # first record
            ...
            elif (tfcfig.record_count / tfcfig.batch_size) % tfcfig.num_accum_grad == 0:  # apply grads to change vars
                if tfcfig.training == 1:
                    ...
                    tfcfig.accum_gradient = [flat_gradients(grad) for grad in gradients if grad is not None]
                    tf.print(f"STEP:{tfcfig.step} AUROC:{auc.result().numpy()} LOSS:{loss.result().numpy()}")
            ...
            tfcfig.record_count += 1
            return loss, auc, tfcfig
    ...
    def np_load(filename):
        ...
        # pot_arr_hb, pot_arr_pi = get_stat_pot(tfconfig, i)
        # make tf.dataset data
        proid_tf = tf.convert_to_tensor(arr["proid"], dtype="float32")
        protok_tf = tf.convert_to_tensor(arr["protok"], dtype="float32")
        rnatok_tf = tf.convert_to_tensor(arr["rnatok"], dtype="float32")
        label_tf = tf.convert_to_tensor(arr["label"], dtype="float32")
        ...
        # proid_tf = tf.data.Dataset.from_tensor_slices(tf.convert_to_tensor(arr["proid"], dtype="int16"))
        # protok_tf = tf.data.Dataset.from_tensor_slices(tf.convert_to_tensor(arr["protok"], dtype="float32"))
        # rnatok_tf = tf.data.Dataset.from_tensor_slices(tf.convert_to_tensor(arr["rnatok"], dtype="float32"))
        # label_tf = tf.data.Dataset.from_tensor_slices(tf.convert_to_tensor(arr["label"], dtype="int16"))
        # pro_mask_tf = tf.data.Dataset.from_tensor_slices(tf.convert_to_tensor(arr["pro_mask"], dtype="float32"))
        # cross_mask_tf = tf.data.Dataset.from_tensor_slices(tf.convert_to_tensor(arr["cross_mask"], dtype="float32"))
        # pot_arr_hb_tf = tf.data.Dataset.from_tensor_slices(tf.convert_to_tensor(arr["hb_pots"], dtype="float32"))
        # pot_arr_pi_tf = tf.data.Dataset.from_tensor_slices(tf.convert_to_tensor(arr["pi_pots"], dtype="float32"))
        # combine
        # dataset = tf.data.Dataset.zip(
        #     (proid_tf, protok_tf, rnatok_tf, pot_arr_hb_tf, pot_arr_pi_tf, pro_mask_tf, cross_mask_tf, label_tf))
        # combined_dataset = combined_dataset
        # combined_dataset = tf.data.Dataset.zip(
        #     ((proid_tf, protok_tf, rnatok_tf, pot_arr_hb_tf, pot_arr_pi_tf, pro_mask_tf, cross_mask_tf), label_tf))
        # combined_dataset = combined_dataset.batch(tfconfig.batch_size)
        # return tf.data.Dataset.zip(
        #     (proid_tf, protok_tf, rnatok_tf, pot_arr_hb_tf, pot_arr_pi_tf, pro_mask_tf, cross_mask_tf, label_tf))
        return (proid_tf, protok_tf, rnatok_tf, pot_arr_hb_tf, pot_arr_pi_tf, pro_mask_tf, cross_mask_tf, label_tf)


    def label_load(filename):
        arr = np.load(filename.numpy(), allow_pickle=True)
        label_tf = tf.convert_to_tensor(arr["label"], dtype="float32")
        return (label_tf)
    ...
    # tf.print(list(npz_list.as_numpy_iterator()))

    # load data
    # combined_dataset = npz_list.map(np_load, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    label_dataset = npz_list.map(lambda x: tf.py_function(func=label_load, inp=[x],
        Tout=[tf.int16]))
    combined_dataset = npz_list.map(lambda x: tf.py_function(func=np_load, inp=[x],
        Tout=[tf.int16, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32]))

    # batchfy
    combined_dataset = combined_dataset.batch(tfconfig.batch_size, drop_remainder=True)
    label_dataset = label_dataset.batch(tfconfig.batch_size, drop_remainder=True)

    # fit_input = np.array(combined_dataset)
    fit_input = [combined_dataset, np.array([tfconfig])]

    # make a model instance, and run it
    model = CustomModel()
    model.compile(optimizer=optimizer)
    model.fit(fit_input, label_dataset, epochs=tfconfig.max_epoch, callbacks=[CheckPoint(), ctensorboard_callback])
    ################################
        # record_num, data_count, loss_sum = 0, 0, 0  # confirmed that cv_id_list starts with 0
        # for i in filenum_range:
        #     # arr = np.load(f"{tfconfig.training_npz_dir}{i}.npz", allow_pickle=True)
        #     for data_combined, label_data in combined_dataset:
        #         tfconfig.proid = data_combined[0]
        #         tfconfig.protok = data_combined[1]
        #         # tfconfig.protok = tfconfig.protok[tf.newaxis, :]
        #         # tf.print(f"tfconfig.protok.shape after {tfconfig.protok}")
        #         tfconfig.rnatok = data_combined[2]
        #         # tfconfig.rnatok = tfconfig.rnatok[tf.newaxis, :]
        #         tfconfig.statpot_hb = data_combined[3]
        #         tfconfig.statpot_pi = data_combined[4]
        #         tfconfig.self_pro_mask_list = data_combined[5]
        #         tfconfig.cross_padding_mask_list = data_combined[6]
        #         tfconfig.label = label_data
        #         model = CustomModel()
        #         loss, auc, loss_sum, tfconfig = model.fit(auc, data_count, tfconfig, loss_sum)
    ...
    # ##############  put the following inside train_step
    #

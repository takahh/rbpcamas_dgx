...
# # THIS IS NOT PRODUCTION. JUST FOR A TIME TEST
# ##########################################
# -------------------------------------------------------------------
...
class Tfconfig():
    ...
    def update(self):
        self.taskname = f"protein_{self.group_to_ignore}_{self.max_batch_num}files_128_version_timetest_corrected"
...
def opt_trfmr(tfconfig):
    ...
    def get_three_veclists(inp):  # inp = [proid_array, feat_array, label_array]
        ...
        cross_padding_mask_list_small = np.concatenate([[[0]*102 + [1]*3] * 102, [[1] * 105] * 3])
...
if __name__ == '__main__':
    ...
    tfconfig.max_batch_num = 58  # 1 epoch data size = 10 * files_per_batch * max_batch_num

...
RNA_SEQ_FILE = f"{BASE_PATH}data/RNA128_test_200/"  # 0.npy
WARMUP_STEPS = 5
...
def opt_trfmr(tfconfig):
    ...
    def get_three_veclists(inp):  # inp = [proid_array, feat_array, label_array]
        ...
        # protein_name = PROTNAMES[inp[0][0][0]]
        ...
        return [pvecs_list_padded, rna_vec_list, label_list, pro_padding_mask_list, rna_padding_mask_list, cross_padding_mask_list, cross_padding_mask_list_small, protein_name_list]


    # @tf.function(input_signature=train_step_signature)
    @tf.function()
    def infer_step(inp, m):  # tar : (100, 30, 170)
        # get protein seqs, and make masks
        tar_inp, inp, labellist, pro_padding, rna_padding, cross_mask, cross_mask_small, protein_names\
            = get_three_veclists(inp)  # protein features vecs
        if tar_inp is None:
            return [None, m]
        ...
        predictions, _ = transformer(inp, tar_inp, True, enc_padding_mask, cross_mask_small, cross_mask)
        ...
        return predictions, labellist, protein_names
    ...
    # 1 EPOCH
    predictions_all, labellist_all, protein_list_all = [], [], []
    for batch_count in range(tfconfig.max_batch_num):
        ...
        predictions, labellist, protein_list = infer_step(train_array, auc)
        if batch_count == 0:
            predictions_all = predictions
            labellist_all = labellist
        else:
            predictions_all = tf.concat([predictions_all, predictions], 0)
            labellist_all = tf.concat([labellist_all, labellist], 0)
        protein_list_all += protein_list

    protein_uniq_list = list(set(np.array(protein_list_all).tolist()))
    # tf.print(tf.nn.softmax(predictions)[:, 1].numpy().shape)
    # tf.print(np.array(labellist_all[0]).shape)
    # tf.print(np.array(protein_list_all).shape)

    result_dict = {"predictions0": tf.nn.softmax(predictions_all)[:, 0].numpy(), "predictions1": tf.nn.softmax(predictions_all)[:, 1].numpy(),
                   "label0": np.array(labellist_all[:, 0]), "protein_name": np.array(protein_list_all)}
    result_df = pd.DataFrame(result_dict)
    auc = tf.keras.metrics.AUC()
    auc_dict = {}
    for proname in protein_uniq_list:
        auc.reset_states()
        target_df = result_df[result_df["protein_name"] == proname]
        auc.update_state(target_df["label0"], target_df["predictions1"])
        auc_dict[proname] = auc.result().numpy()
    print(auc_dict)
...
if __name__ == '__main__':
    ...
    tfconfig.max_batch_num = 10  # 1 epoch data size = 10 * files_per_batch * max_batch_num

...
class Tfconfig():
    ...
    def update(self):
        self.pairfile = f"{BASE_PATH}data/benchmarks/label/{self.benchname}_pairs.txt"
        self.protein_seq_file = f"{BASE_PATH}data/protein768/{self.benchname}"  # AARS.npy
        self.rna_seq_file = f"{BASE_PATH}data/RNA768_training_200/{self.benchname}"  # 0.npy
        self.cv_list_file = f"{BASE_PATH}data/benchmarks/id_list_{self.benchname}_for_5CV.csv.npz"
        self.taskname = f"protein_{self.group_to_ignore}_{self.max_batch_num}files_input768_lr{self.init_lr}_1027"
...
def opt_trfmr(tfconfig):
    ...
    def loss_function(labellist, predictions):
        ...
        tf.print("new_labellist for loss")
        tf.print(new_labellist)
        tf.print("predictions for loss")
    ...
    def train_step(inp, m):  # tar : (100, 30, 170)
        ...
        tf.print("predictions for auc (tf.nn.softmax(predictions)[:, 1].numpy())")
        tf.print(tf.nn.softmax(predictions)[:, 1].numpy())
        tf.print("labellist for auc")
        tf.print(labellist)

...
class Tfconfig():
    ...
    def update(self, task_id):
        ...
        self.training_npz_dir = f"{BASE_PATH}data/training_data_tokenized/{self.group_to_ignore}_small/"
...
def opt_trfmr(tfconfig):
    ...
    combined_dataset_test = npz_list_test.map(lambda x: tf.py_function(func=np_load, inp=[x],
        Tout=[tf.int16, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.int16]))
    # batchfy
    combined_dataset_train = combined_dataset_train.repeat(1000).batch(tfconfig.num_of_gpu, drop_remainder=True)
    combined_dataset_test = combined_dataset_test.batch(tfconfig.num_of_gpu, drop_remainder=True)

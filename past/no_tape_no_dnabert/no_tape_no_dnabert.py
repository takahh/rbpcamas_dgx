...
class Encoder(tf.keras.layers.Layer):  # RNA
    def __init__(self, num_layers, d_model, num_heads, dff, encfg, rate=0.1):
        super(Encoder, self).__init__()
        self.embedding = tf.keras.layers.Embedding(5, d_model)  # 5 because 4 bases and an unknown
        self.d_model = tf.cast(d_model, tf.float32)
        self.pos_encoding = positional_encoding(encfg.max_rna_len, self.d_model)
    ...
    def call(self, training, tf_cfg):
        ...
        ######
        x = self.embedding(np.array([RCODES.index(x) if x in RCODES else 4 for x in tf_cfg.rnaseq]))  # (batch_size, input_seq_len, d_model)
        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float32))
        zero_pad = np.array([[0] * tf_cfg.d_model] * (tf_cfg.max_rna_len - x.shape[0]))
        x = tf.concat([x, zero_pad], axis=0)  # x=[99,64], zero_pad=[3901,1]
        x += self.pos_encoding[:len(tf_cfg.rnaseq), :]
        ######
...
class Decoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, decfg, rate=0.1):
        ...
        self.embedding = tf.keras.layers.Embedding(21, d_model)
        self.pos_encoding = positional_encoding(decfg.max_pro_len, decfg.d_model)
        self.d_model = d_model
        self.num_layers = num_layers
    ...
    def call(self, training, cfg):  # x = tar previously
        ...
        ######
        x = self.embedding(np.array([PCODES.index(x) if x in PCODES else 20 for x in cfg.proseq]))  # (batch_size, input_seq_len, d_model)
        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float32))
        zero_pad = np.array([[0] * self.d_model] * (cfg.max_pro_len - x.shape[0]))
        x = tf.concat([x, zero_pad], axis=0)
        x += self.pos_encoding[:len(cfg.proseq), :]
        ######
...
def opt_trfmr(tfconfig):
    ...
    def train_step(inp, m, record_count, tfcfig, losssum):
        ...
        losssum += loss2.numpy()
        # gradients = tf.convert_to_tensor(gradients[0]) + gradients[1:]

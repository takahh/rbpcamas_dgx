...
rna_to_train_dir = f"{path}RNA128_training/"  # 0.npy
rna_to_test_dir = f"{path}RNA128_test/"  # 0.npy
    ...
    for j in range(3, 5):
        ...
        for i in range(1):
            ...
            for k in range(len(train_list) // 10):
                tf.io.write_file(f"{rna_to_train_dir}{j}/{x}.npy", tf.Tensor(train_list[10 * k: 10 * (k + 1)]))
                x += 1
            #  write test set
            if test_list is not None:

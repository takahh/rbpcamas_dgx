...
class Tfconfig():
    ...
    def update(self):
        self.taskname = f"protein_group_except_{self.group_to_ignore}_{self.max_batch_num}files_light_version"
...
def opt_trfmr(tfconfig):
    ...
    def loss_function(labellist, predictions):
        ...
        lossvalue = tf.cast(bce(new_labellist, predictions), dtype="float32")
        return lossvalue
    ...
    def get_three_veclists(inp):
        ...
        for item in inp:
            try:
                ...
                pvecs = tf.convert_to_tensor(protein_seq_dict[PROTNAMES[proid]][0], dtype=tf.float32)
        ...
        label_list = tf.cast(label_list, dtype="float32")

...
class Tfconfig():
    ...
    def update(self):
        self.taskname = f"protein_group_except_{self.group_to_ignore}_{self.max_batch_num}files_much_light_version"
...
class Transformer(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
        ...
        self.pre_encoder = tf.keras.layers.Dense(d_model)
        self.pre_decoder = tf.keras.layers.Dense(d_model)
    ...
    def call(self, inp, tar, training, enc_padding_mask,
        ...
        inp = self.pre_encoder(inp)
        tar = self.pre_decoder(tar)
...
def opt_trfmr(tfconfig):
    ...
    def loss_function(labellist, predictions):
        ...
        lossvalue = tf.cast(bce(new_labellist, predictions), dtype="float16")
        return lossvalue
    ...
    for epoch in range(max_epoch):
        ...
        for batch_count in range(tfconfig.max_batch_num):
            ...
            for i in range(FILES_PER_BATCH):
                array_to_add = np.load(f"{RNA_SEQ_FILE}{FILES_PER_BATCH * batch_count + i}.npy", allow_pickle=True)
                if i == 0:
...
if __name__ == '__main__':
    ...
    tfconfig.max_batch_num = int(sys.argv[2])

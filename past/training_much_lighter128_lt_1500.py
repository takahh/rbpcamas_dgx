...
RNA_SEQ_FILE = f"{BASE_PATH}data/RNA128_training_1520/"  # 0.npy
WARMUP_STEPS = 5
...
class Tfconfig():
    ...
    def update(self):
        self.taskname = f"protein_{self.group_to_ignore}_{self.max_batch_num}files_128_version"
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inp, tar, training, enc_padding_mask,
        ...
        dec_output, attention_weights = self.decoder(tar, enc_output, training, dec_padding_mask, enc_padding_mask)
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, q, k, mask, training, wordlen=None, block_num=None, mha_num=None):
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training,  # x:RNA, enc_output:protein
             pro_padding_mask, cross_padding_mask, block_num=None):  # tar, enc_output, training, look_ahead_mask, dec_padding_mask)
        # if block_num == 0:
        #     # self-attention part
        #     attn1, attn_weights_block1 = self.mha1(x, x, x, pro_padding_mask, training, block_num, 1)
        ...
        # v, k from RNA, q from protein
        attn2, attn_weights_block2 = self.mha2(  # v, k, q, mask,
            # (batch_size, target_seq_len, d_model)
            out1, out1, enc_output, cross_padding_mask, training, block_num, 2)
        attn2 = self.dropout2(attn2, training=training)  # (49, 105, 768)
        # attn2
        # TensorShape([40, 105, 768])
        # out1
        # TensorShape([40, 3000, 768])
        if block_num == 0:
            out2 = attn2
        else:
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training,  # tar, enc_output,
             pro_padding_mask, cross_padding_mask):  # x = tar previously
        attention_weights = {}
        x = tf.cast(x, tf.float16)
        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float16))
        x /= tf.cast(self.d_model, tf.float16)
        ...
        for i in range(self.num_layers):
            ...
            x, block1 = self.dec_layers[i](x, enc_output, training,
                                                   pro_padding_mask, cross_padding_mask, i)
    ...
    def get_three_veclists(inp):
        ...
        for item in inp:
            try:
                ...
                # if PROTNAMES[item[0][0]] in tfconfig.ignorelist:
                proid = abs(item[0][0])
                if PROTNAMES[proid] in tfconfig.ignorelist:
                    continue
                    # return [None, 0, 0, 0, 0, 0]
                pvecs = tf.convert_to_tensor(protein_seq_dict[PROTNAMES[proid]][0], dtype=tf.float16)
                zeros = tf.convert_to_tensor([[0] * 128] * (1520 - len(pvecs)), dtype=tf.float16)
        ...
        label_list = tf.cast(label_list, dtype="float16")
    ...
    for epoch in range(max_epoch):
        ...
        for batch_count in range(tfconfig.max_batch_num):
            ...
            for i in range(tfconfig.files_per_batch):
                array_to_add = np.load(f"{tfconfig.datapath}{tfconfig.files_per_batch * batch_count + i}.npy", allow_pickle=True)
                if i == 0:
                    train_array = array_to_add
                else:
                    train_array = np.concatenate((train_array, array_to_add))  # (50, 3,)
...
if __name__ == '__main__':
    tfconfig = Tfconfig()
    tfconfig.init_lr = 1e-20
    tfconfig.num_layers = 3
    tfconfig.num_heads = 2
    tfconfig.max_batch_num = 1160
...
def study3_train(trial):  # for optuna
    tfconfig = Tfconfig()
    tfconfig.init_lr = 10 ** (-trial.suggest_int("lr", 5, 22))

...
class Tfconfig():
    def __init__(self):
        ...
        self.self_pro_num_layers = SELF_PRO_NUM_LAYERS
    ...
    def update(self):
        self.taskname = f"protein_{self.group_to_ignore}_{self.max_batch_num}files_128_protein_self_version"
...
class Transformer(tf.keras.Model):
    def __init__(self, num_layers, self_pro_num_layers, d_model, num_heads, dff, input_vocab_size,
                 target_vocab_size, pe_input, pe_target, rate=0.1):
        super(Transformer, self).__init__()
        self.pro_self_encoer = Encoder(self_pro_num_layers, d_model, num_heads, dff, target_vocab_size, pe_input, rate)
        self.encoder = Encoder(num_layers, d_model, num_heads, dff, input_vocab_size, pe_input, rate)
        self.decoder = Decoder(num_layers, d_model, num_heads, dff, target_vocab_size, pe_target, rate)
        self.final_layer = FinalLayer()

    def call(self, inp, tar, training, enc_padding_mask, dec_padding_mask, cross_padding_mask, wordlen=None):
        ...
        pro_self_out = self.pro_self_encoer(tar, training, enc_padding_mask, wordlen)
        dec_output, attention_weights = self.decoder(pro_self_out, enc_output, training, dec_padding_mask, enc_padding_mask)
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, input_vocab_size,
        ...
        #        tar, training, enc_padding_mask, wordlen
    def call(self, x, training, mask, wordlen=None):
        #         inp, tar,   training, enc_padding_mask, dec_padding_mask
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, pro_padding_mask, cross_padding_mask):
        attention_weights = {}
        x = tf.cast(x, tf.float16)
        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float16))
        x /= tf.cast(self.d_model, tf.float16)
        ...
        for i in range(self.self_pro_num_layers):
            x, block1 = self.dec_self_layers[i](x, enc_output, training,
...
def opt_trfmr(tfconfig):
    ...
    pro_self_num_layers = tfconfig.self_pro_num_layers
    ...
    transformer = Transformer(num_layers, pro_self_num_layers, d_model, num_heads, dff,
    ...
    def get_three_veclists(inp):  # inp = [proid_array, feat_array, label_array]
        ...
        cross_padding_mask_list = np.array([[[x] * 102] for x in pro_padding_mask_list])

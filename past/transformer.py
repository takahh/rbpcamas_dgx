...
def scaled_dot_product_attention(k, q, v, tffig, one_on_rna,
    ...
    if one_on_rna == 2 and tffig.use_attn_augument == 1:
        table_to_augment = tffig.statpot_hb
        ...
        if scaled_attention_logits.shape[-1] != table_to_augment.shape[-1]:
            table_to_augment = tf.transpose(table_to_augment)
            table_to_augment_pi = tf.transpose(table_to_augment_pi)
        scaled_attention_logits += w_hb * tf.cast(table_to_augment, dtype="float32")
        scaled_attention_logits += w_pi * tf.cast(table_to_augment_pi, dtype="float32")
...
class RepeatLayers(tf.keras.layers.Layer):
    def __init__(self, trscfg):
        ...
        for i in range(trscfg.layer_num):
            if i == 0:
                self.rencoder = EmbAttention(trscfg, 1)
                self.pencoder = EmbAttention(trscfg, 0)
            self.cross_attention = CrossAttention(trscfg)

    def call(self, tfcfg):
        rna_out, pro_out = None, None
        for i in range(tfcfg.layer_num):
            if i == 0:
                rna_out = self.rencoder.call(tfcfg, 1, rna_out)
                pro_out = self.pencoder.call(tfcfg, 0, pro_out)
...
class EmbAttention(tf.keras.layers.Layer):  # RNA
    def __init__(self, encfg, one_if_rna):
        ...
        if self.one_if_rna == 1:
            ...
            self.vocab_size = 20
        self.embedding = tf.keras.layers.Embedding(self.vocab_size + 1, encfg.d_model)  # 5 because 4 bases and an unknown
    ...
    def call(self, tf_cfg, one_when_rna, x=None):  # tfcfg, RCODES, i, 1, self.rna_out
        # embedding
        if one_when_rna == 1:
            x = self.embedding(tf_cfg.rnatok)
            self.maxlen = tf_cfg.max_rna_len
        elif one_when_rna == 0:
            x = self.embedding(tf_cfg.protok)
            self.maxlen = tf_cfg.max_pro_len
        x *= tf.math.sqrt(tf.cast(self.d_model, tf.float32))  # x=(1, 662, 64)
        # padding
        if one_when_rna == 0:
            zero_pad = np.array([[0] * tf_cfg.d_model] * (self.maxlen - x.shape[1]))
...
def get_three_veclists(tffg):
    ...
    # 2. make masks
    # protein self mask
    protein_length = len(tffg.protok)
    ...
    lower_mask = np.array([[1] * tffg.max_rna_len] * (tffg.max_pro_len - protein_length))
...
def opt_trfmr(tfconfig):
    ...
    for epoch in range(max_epoch):
        ...
        if epoch % 10 == 0:
            ckpt_manager.save()
        tf.print(f"EPOCH:{epoch} AUROC:{auc.result().numpy()} LOSS:{epoch_loss.result().numpy()} LR:{optimizer.lr.numpy()}")
...
if __name__ == '__main__':
    ...
    config.use_self = int(sys.argv[5])
    config.use_layernorm = int(sys.argv[6])
    config.layer_num = int(sys.argv[7])
    config.two_d_softm = int(sys.argv[8])
    config.num_heads = int(sys.argv[9])
    config.max_file_num = int(sys.argv[10])
    config.usechpoint = int(sys.argv[11])
    config.init_lr = 10 ** (-int(sys.argv[12]))
    tf.print(config.__dict__)
    config.update()
    opt_trfmr(config)

...
def scaled_dot_product_attention(k, q, v, tffig, one_on_rna):
    ...
    ########################################################
    # ----------------------------------------------------------------------
    # maybe I should apply 2d-softmax only to cross cell (one_on_rna == 2) ??
    # ----------------------------------------------------------------------
...
def opt_trfmr(tfconfig):
    ...
    # combined_dataset_validation = combined_dataset.skip(int(16)).skip(int(2)).take(int(2))
    ...
    combined_dataset_train = combined_dataset_train.repeat(1).batch(dataset_batch).prefetch(tf.data.AUTOTUNE).with_options(options)
    ...
    combined_dataset_test = combined_dataset_test.repeat(1).batch(dataset_batch).prefetch(tf.data.AUTOTUNE).with_options(options)
    if tfconfig.data_mode != "lnc_unknown":

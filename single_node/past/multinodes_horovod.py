...
import json, sys
# sys.path.append("/gs/hs0/tga-science/kimura/transformer_tape_dnabert/.pyenv/versions/horoenv/lib/python3.8/site-packages/")

import tensorflow as tf
# session = tf.compat.v1.Session()

import horovod.tensorflow as hvd
import horovod.tensorflow.keras as hvd2
...
def opt_trfmr(tfconfig):
    ...
    class CustomModel(tf.keras.Model):
        ...
        def train_step(self, data_combined):
            ...
            tfconfig.proid, tfconfig.protok, tfconfig.rnatok, tfconfig.statpot_hb, tfconfig.statpot_pi, \
            tfconfig.self_pro_mask_list, tfconfig.cross_padding_mask_list, tfconfig.label, tfconfig.p_tape_tf = \
            map(self.flatten_n_batch, datalist)
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate * hvd.size(), beta_1=0.9, beta_2=0.98, epsilon=1e-9)
    optimizer = hvd.DistributedOptimizer(optimizer)
    current_time = datetime.datetime.now().strftime("%Y%m%d")
    checkpoint_path_daily = f"{tfconfig.checkpoint_path}_{current_time}" if hvd.rank() == 0 else None
    checkpoint_path_integrated = f"{tfconfig.checkpoint_path}" if hvd.rank() == 0 else None
    ...
    cp_callback_daily = tf.keras.callbacks.ModelCheckpoint(checkpoint_path_daily,
                                                     monitor="eval_loss",
                                                     mode="min",
                                                     save_weights_only=True,
                                                     verbose=1)
    cp_callback_integrated = tf.keras.callbacks.ModelCheckpoint(checkpoint_path_integrated,
                                                     monitor="eval_loss",
                                                     mode="min",
                                                     save_weights_only=True,
                                                     verbose=1)
    ...
    if tfconfig.training == 1:
        ...
        combined_dataset_train = combined_dataset_train.repeat(1).shard(num_shards=hvd.size(), index=hvd.rank()).cache()\
            .batch(dataset_batch, drop_remainder=True).prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
        ...
        combined_dataset_test = combined_dataset_test.repeat(1).shard(num_shards=hvd.size(), index=hvd.rank()).batch(dataset_batch)
        ...
        combined_dataset_test = combined_dataset_test.repeat(1)
        # combined_dataset_test = combined_dataset_test.repeat(1).batch(dataset_batch).with_options(options)
        # distribute the tf.dataset
        # combined_dataset_test = strategy.experimental_distribute_dataset(combined_dataset_test)
    # run model
    ...
    if tfconfig.training == 1:
        if hvd.rank() == 0:
            callbacks = [cp_callback_daily, cp_callback_integrated, CustomCallback(), hvd2.callbacks.BroadcastGlobalVariablesCallback(0)]
        else:
            callbacks = [CustomCallback(), hvd2.callbacks.BroadcastGlobalVariablesCallback(0)]
    elif tfconfig.training == 0:
        callbacks = [CustomCallback()]

    if tfconfig.usechpoint == 1 and hvd.rank() == 0:
        model.load_weights(tfconfig.cp_callback_integrated)
    if tfconfig.training == 1:

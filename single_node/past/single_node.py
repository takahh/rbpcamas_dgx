...
def scaled_dot_product_attention(k, q, v, tffig, one_on_rna):
    ...
    if one_on_rna == 2 and tffig.use_attn_augument == 1:  # in a cross layer and the aug flag is on
        ...
        # augment
        tf.print(f"coeff to the 1st attn matrx {tffig.aug_weight_at}")
        tf.print(f"coeff to the stat pot matrx {tffig.aug_weight_aug}")
        scaled_attention_logits *= tffig.aug_weight_at
        attn_weight_before = scaled_attention_logits
        if tffig.aug_multiply == 0:
            stats_to_add = tffig.aug_weight_aug * tf.cast(table_to_augment, dtype="float32")
